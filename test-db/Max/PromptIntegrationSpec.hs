...
spec pool = before_ (truncateAll pool) $
  describe "Max.Prompt.buildContext (integration)" $ do
    it "renders ambient rows from the messages table" $ do
      ...
      (msgs, notes) <-
        withDbLog pool $ buildContext "default-persona" 20 False False "var/images" utc [] s trigger
      notes `shouldBe` []
    ...
    it "honours cleared_at watermark — older rows are dropped" $ do
      ...
      (msgs, _) <- withDbLog pool $ buildContext "default-persona" 20 False False "var/images" utc [] s trigger
      let ub = userBodyOf msgs
    ...
    it "renders reply context when trigger has SegReply" $ do
      ...
      (msgs, _) <- withDbLog pool $ buildContext "default-persona" 20 False False "var/images" utc [] s replyTrigger
      let ub = userBodyOf msgs
    ...
    it "drains btw notes into the second tuple element" $ do
      ...
      withDb pool $ upsertSession s
      (_, notes) <- withDbLog pool $ buildContext "default-persona" 20 False False "var/images" utc [] s trigger
