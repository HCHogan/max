...
psql -X -v ON_ERROR_STOP=1 -1 -d "$database" -f migrations/089_browser_workspaces.sql >/dev/null
psql -X -v ON_ERROR_STOP=1 -d "$database" -f test-db/fixtures/task-upgrade-after.sql >/dev/null
psql -X -v ON_ERROR_STOP=1 -d "$database" -f test-db/fixtures/browser-upgrade-after.sql
