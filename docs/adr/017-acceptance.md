...
| 4. One event admission, routing policy and wake rule | Pending | Frontend, relay and monitor producers are traced below. Complete the architectural review of routing policy currently split between Conversation/Router/Routing, and native/guest awaited completions that wake through `Execution.Tools.awaitWake` rather than a delivered `Events.Settled`. Also account for child-report joins through `Node.Futures` and guest reply/resume TMVars; these currently bypass the event wake predicate. Do not infer this invariant from the passing frontend/relay tests. |
