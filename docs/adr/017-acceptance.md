...
| 4. One event admission, routing policy and wake rule | Pending | Frontend recipient selection, completion/message buffering versus relay/fold, and monitor queue/coalesce/overflow now use the single typed `Node.Routing.route` policy. Conversation, Router and the locked occurrence store supply facts and apply its decision in the same transaction. RoutingSpec covers canonical ordering, quoted/queued/closed owners and delivery kinds; Conversation/Router/Jobs and DB monitor specs exercise the adapters, including folding a normal late tell under full relay capacity. Remaining: native awaited completions wake through `Execution.Tools.awaitWake` rather than a delivered `Events.Settled`, and selected child-report joins use `Node.Futures`. Audit guest step/reply/resume scheduling separately: §4 explicitly excludes guest-private tool outcomes from model Settled events, so unifying readiness must not duplicate those outcomes into model observations. Do not infer this invariant from the passing routing tests. |
