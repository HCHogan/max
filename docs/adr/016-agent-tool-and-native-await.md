...
> [ADR-017](017-async-tools-and-agent-executors.md) replaces the blocking guest
> and batch barrier with poll-driven calls, removes the leaf-worker rule, and
> cancels outstanding calls when a program returns. Its later executor steps
> will release conversations during waits.
