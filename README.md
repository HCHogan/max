...
- **Tools** (registered conditionally on config):
  ...
  - Sandbox tools — persistent per-group Docker workspace for code execution. Nix-based: containers run `max-sandbox:latest` (build once with `sandbox-image/build.sh`), packages come from the pinned nixpkgs 26.05 via `nix_search` + `sandbox_exec`'s `packages` arg, and all sandboxes share one store through the `max-nix` volume so any package is downloaded at most once. The store volume survives restarts; reclaim space with `nix store gc` in a container while nothing runs, or `docker volume rm max-nix` to re-seed after rebuilding the image with a new pin.
...
sandbox-image/     nix-enabled sandbox base image (nixpkgs 26.05 pin, TUNA
                   substituter, no auto-GC) — build.sh → max-sandbox:latest
...
src/Max/           Config (opt-env-conf: CLI/env/YAML), Forward worker, Image worker pool,
...
The only required value is one LLM `api_key`; everything else has a default. See `.env.example` and `max.yaml.example` for the full schema, including the `llm.profiles` map used for multi-model setups. Run `max-bot --help` for the full option/env/config reference (generated by opt-env-conf).
...
Non-secret bits (persona, history window, profile list) are easier to keep in `max.yaml` — copy `max.yaml.example` and uncomment.
