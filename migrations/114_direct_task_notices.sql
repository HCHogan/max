-- Retire model review without reviving previously skipped notifications.
UPDATE task_notifications SET superseded_at=COALESCE(superseded_at,clock_timestamp())
WHERE review_decision->>'action'='skip' AND delivered_at IS NULL;

-- Publication still checks the current task, conversation owner and receipt.
