...
python（省一套 venv）；要精细读写内容才上 python 库——记住 sandbox 手册里的
Python 规则：packages=["python3"] 建 venv 再 pip install，nixpkgs 的
python3Packages.* 是 import 不到的。
...
表格抽取 pdftotext 常常糊，pip 装 pdfplumber 按页 extract_tables。
...
pip 装 openpyxl。读：`load_workbook(path, read_only=True, data_only=True)`——
...
pip 装 python-docx。读：`Document(path)` 遍历 `.paragraphs` 和 `.tables`。
...
pip 装 python-pptx。生成：`Presentation()` →
