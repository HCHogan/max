...
helpText Nothing =
  T.unlines
    [ "可用命令：",
      ...
      "  !agent list/status/steer/cancel/replace  操作当前进程的子 agent（agent#N）；replace 直接提供新目标",
      "  !feedback <text>        回复任务消息时补充该任务，否则补充自己最新的开放任务（别名 !fb）",
      "  !feedback agent#N <text>  给指定子 agent 补一句；普通回复按引用消息定位前台任务",
      "  !memory                  看本群的长期记忆",
