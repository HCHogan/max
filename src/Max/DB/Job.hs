...
module Max.DB.Job (allocateJobId, admitFromTurn) where
...
admitFromTurn jobs turn spec = do
  ...
  if live && authorized
    then do
      ...
      liftIO (Jobs.admitJob jobs (Just turn.atrTurnId) identifier spec)
