...
parseCatalog = either (const (Left "maxops returned an invalid or unsupported operation catalog")) Right . parseEither parser
  where
    parser = withObject "maxops catalog" $ \fields -> do
      ...
      cursor <- fields .:? "next_cursor" :: Parser (Maybe Text)
      unless (isNothing cursor) (fail "partial catalog")
      ...
      unless (Set.size (Set.fromList names) == length names && Set.size (Set.fromList (map operationToolName operations)) == length names && length names <= 128) (fail "duplicate operations")
    ...
    parseOperation version = withObject "operation" $ \fields -> do
      ...
      unless (not (T.null name) && T.length name <= 55 && T.all (\c -> isAsciiLower c || isDigit c || c `elem` ['.', '_']) name && isObject schema) (fail "invalid operation")
      if version == 1
        then pure $ if readOnly then Just (Operation name True False (Object fields)) else Nothing
          ...
          response <- fields .:? "response_schema" .!= Object mempty
          ...
          pure (Just (Operation name readOnly (idempotency == "required") (Object (KeyMap.delete "response_schema" fields))))

    isObject (Object _) = True
    isObject _ = False

catalogValue :: CatalogAccess -> Catalog -> Value
catalogValue access catalog =
  object
...
-- | Keep pagination metadata while replacing binary envelopes with readable
-- text, including chunks split inside UTF-8.
...
operationToolName :: Operation -> Text
operationToolName entry = "maxops_" <> T.replace "." "_" entry.name

operationSchema :: Operation -> Value
operationSchema entry = case entry.wireValue of
  Object fields -> fromMaybe (Object mempty) (KeyMap.lookup "params_schema" fields)
  _ -> Object mempty

operationSummary :: Operation -> Text
operationSummary entry = case entry.wireValue of
  Object fields | Just (String summary) <- KeyMap.lookup "summary" fields -> summary
  _ -> entry.name
