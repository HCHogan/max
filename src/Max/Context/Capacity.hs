...
import Max.ModelCatalog (ContextLimits, contextWorkingBudget)

-- Up-front capacities follow the working budget: a soft context_budget when
-- configured, otherwise the hard input budget.
rawHighTokens, rawLowTokens, summaryTokens, readPageTokens :: ContextLimits -> Bool -> Int
rawHighTokens limits media = contextWorkingBudget limits media `div` 2
rawLowTokens limits media = contextWorkingBudget limits media `div` 4
summaryTokens limits media = contextWorkingBudget limits media `div` 8
-- The byte ceiling is a transport guard, not a model-window default.
readPageTokens limits media = max 1 (min 16384 (contextWorkingBudget limits media `div` 32))
