...
import Max.Tool.Arguments qualified as Args (int64, required)
import Max.Tool.Protocol (argumentTool, committedResult)
...
pinTool =
  argumentTool
    ...
    (Args.required "message_id" (Args.int64 "要固定的消息 id（上下文行里的 #<id>）"))
    $ \mid ->
      committedResult . renderResult mid <$> pinMessage mid
...
unpinTool =
  argumentTool
    ...
    (Args.required "message_id" (Args.int64 "要移除的消息 id（[pinned] 行里的 #<id>）"))
    $ \mid ->
      committedResult . renderResult mid <$> unpinMessage mid
