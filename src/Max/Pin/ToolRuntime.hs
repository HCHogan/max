...
import Max.Effects.PinControl (PinControlScope (..), runPinControlWithAuthority)
...
pinToolsWithDatabase sessions defaultModel context = map (hoistTool (runPinControlWithAuthority (toolCallAuthority context) scope sessions defaultModel)) pinToolsFor
  where
