...
returnTypes =
  Map.fromList
    [ ("inspect_source", "{git_revision: string; bundle_hash: string; file_count: number; source_bytes: number} & ({action: \"search\"; query: string; results: {path: string; line: number; snippet: string}[]} | {action: \"read\"; path: string; start_line: number; end_line: number; total_lines: number; next_line: number | null; content: string} | {action: \"tree\"; path: string; files: string[]; truncated: boolean})"),
      ...
      ("context_read", "{items: (Msg | Memory | Observation)[]; prev: {cursor: string} | null; next: {cursor: string} | null; anchor?: string | null; episode?: {ref: string; start_cursor: string; end_cursor: string; state: string; source_hash_matches: boolean} | null; order?: \"ingest\" | \"forward_position\"}；Msg = {kind: \"message\"; ref: string; sender: {id: string; name: string}; received_at: string; occurred_at: string; text: string; text_offset: number; complete: boolean; more: {cursor: string} | null; reply_to: string | null; episode: string | null; forward: {ref: string} | null; message_kind: string; prompt_eligible: boolean; in_episode?: boolean}；Observation = {kind: \"node_observation\"; text: string; text_offset: number; complete: boolean; more: {cursor: string} | null}；Memory = {kind: \"memory\"; ref: string; text: string; version: string; lifecycle: string; subject: string; subject_id: string; updated_at: string; evidence: {kind: string; note: string | null; message: {ref: string} | null; episode: {ref: string} | null}[]}"),
      ...
      ("agent_tell", "{delivered: true}"),
      ("agent_ask", "{author: number; source_message: number | null; body: string}；body 是上级的回答。原生等待被中断时返回执行句柄，可用 execution_wait 等待。"),
