...
returnTypes =
  Map.fromList
    [ ("inspect_source", "{git_revision: string; bundle_hash: string; file_count: number; source_bytes: number} & ({action: \"search\"; query: string; results: {path: string; line: number; snippet: string}[]} | {action: \"read\"; path: string; start_line: number; end_line: number; total_lines: number; next_line: number | null; content: string} | {action: \"tree\"; path: string; files: string[]; truncated: boolean})"),
      ...
      ("poke", "{ok: true}"),
      ("set_reminder", "{ok: true; handle: string; next_fire: string; recurring: boolean; cron: string | null}"),
      ("list_reminders", "{handle: string; text: string; next_fire: string; cron: string | null; fire_count: number}[]"),
      ("cancel_reminder", "{ok: true; revision: number; pending_policy: string; admitted_tasks_cancelled: boolean}"),
      ("arm_monitor", "{ok: true; handle: string; trigger: \"time\" | \"ledger\" | \"http\"; next_fire?: string | null; recurring?: boolean; cron?: string | null; url?: string; bearer_token?: string; method?: \"POST\"; max_body_bytes?: number}"),
      ("list_monitors", "{handle: string; goal: string; trigger: string; continuation: string; next_fire: string | null; expires: string | null; fire_count: number; max_fires: number | null}[]"),
      ("cancel_monitor", "{ok: true; revision: number; pending_policy: string; admitted_tasks_cancelled: boolean}"),
      ("configure_monitor", "{ok: true; revision: number; profile: string; change_only: boolean; pending_policy: string; admitted_tasks_cancelled: boolean}"),
      ("monitor_history", "{handle: string; revision: number; goal: string; profile: string; status: \"armed\" | \"fired\" | \"cancelled\" | \"expired\"; overlap: string; queue_limit: number; change_only: boolean; next_fire: string | null; fires: {fire_id: number; definition_revision: number; scheduled_at: string; disposition: string; task_id: number | null; coalesced_into: number | null; admission_state: \"pending\" | \"dispatched\"; evidence: string; last_error: string | null}[]}"),
      ...
      ("task_start", "Task；Task = {task: string; objective: string; profile: string; owner: number; group_id: number; parent: string | null; status: string; progress: string | null; result: unknown; calls: number; model_rounds: number; deadline: string}"),
      ("task_list", "Task[]（字段同 task_status）"),
      ("task_status", "{task: string; objective: string; profile: string; owner: number; group_id: number; parent: string | null; status: string; progress: string | null; result: unknown; calls: number; model_rounds: number; deadline: string}"),
