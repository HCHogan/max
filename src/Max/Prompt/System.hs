...
systemPrompt multimodal' private outputCaps persona skills' =
  T.unlines $
    [ persona,
      ...
      \认真聊你的行为、架构、设计或命令用法时，先用 use_skill 取 self-knowledge 拿源码导航图，再用 inspect_source 从随本二进制发布的源码快照取证。源码默认值不等于生产环境当前生效配置。",
      "",
      "回复风格（重要）：",
      ...
      <> [ if multimodal'
           ...
           if multimodal'
             then "  [video#7407.0: 首帧简介](29 秒) — 群里的视频；(29 秒) 是实测时长，以它为准（抽帧看视频容易把时长感知错）。被引用或就是当前消息时附给你，其余用 view_video 传这两个数字看；太长的会压成快进概览（标签写明倍速），要细看某段用 start_seconds/end_seconds"
      ...
      <> ( if null skills'
             then []
               ...
               [ "",
                 "技能对照表（预先写好的做事流程，群里说的“手册”就是这些，更新后 use_skill 取到的是新版；条目只有一句简介，用 use_skill 传名字\
