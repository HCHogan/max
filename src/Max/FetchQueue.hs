...
-- missing work; no job payload, lease or restart attempt is stored in SQL.
...
runFetchLoop :: (Log :> es, IOE :> es) => FetchSignal -> JobKind a -> (a -> Eff es (Either Text ())) -> Eff es ()
runFetchLoop signal kind process =
  forever $
    bracket
  ...
  where
    ...
    attempt key payload count = do
      ...
      case either (Left . T.pack . show) id result of
        Right () -> pure ()
        Left err -> do
          logAttention "media fetch failed" (object ["kind" .= kindName kind, "key" .= key, "attempt" .= count, "error" .= err])
          if count < (5 :: Int)
            then do
              liftIO (threadDelay (250_000 * 2 ^ (count - 1)))
              attempt key payload (count + 1)
            else liftIO . atomically $ modifyTVar' queue.failures (+ 1)
