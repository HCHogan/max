...
data TaskControl :: Effect where
  ...
  ControlTask :: DurableTaskId -> TaskCommand -> TaskControl m (Either TaskControlError TaskControlReceipt)
...
controlTask :: (TaskControl :> es) => DurableTaskId -> TaskCommand -> Eff es (Either TaskControlError TaskControlReceipt)
...
runTaskControl scope = interpret $ \_ -> \case
  ...
  ControlTask identifier command -> case scope.turn of
        ...
        then pure (Left TaskCallerFenced)
          ...
          if scope.background
            then case command of
              SteerTask note -> steerChildTyped turn.atrTurnId identifier.unDurableTaskId note
              _ -> pure (Left TaskChildScopeRequired)
            else DB.controlTask scope.group scope.principal False identifier command Nothing
