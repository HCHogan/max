...
-- | Capacity and duplicate checks share the commit lock used by every live
-- producer. Keep this inside a larger publication transaction when needed.
admitMemory :: (WithConnection :> es, IOE :> es) => DuplicatePolicy -> MemoryActor -> MemoryNamespace -> MemoryDraft -> Eff es (Either MemoryAdmissionFailure MemoryItem)
admitMemory duplicates actor namespace draft = withTransaction $ do
  let (_, _, group) = namespaceParts namespace
