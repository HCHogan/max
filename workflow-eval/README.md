...
`runAgentRuntime`, process-owned Jobs, shared root budgets, phase/join callbacks
and diagnostic journal. Normal final text completes a child; explicit output
contracts are validated by the host. A frozen `web_search` reader serves the exact
...
no work resumes after restart. A timeout cancels the guest and local workers.
Successful parents complete after all child results have been collected. The report is updated after each arm and retains source fingerprints,
...
Completion requires the guest to finish and all three child results to be
