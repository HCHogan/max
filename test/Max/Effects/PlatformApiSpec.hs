...
import Max.Effects.PlatformApi (callQQActionOnGeneration, withPendingCall)
...
spec :: Spec
spec = do
  describe "PlatformApi pending calls" $ do
  ...
  describe "generation-fenced QQ calls" $ do
    it "fails without touching a client from another websocket generation" $ do
      ...
      case result of
        Left err -> err `shouldBe` "connection generation changed"
