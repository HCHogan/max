...
plain = formatLogMessage utc False

spec :: Spec
spec = do
  timezoneSpec
  describe "Max.Log.formatLogMessage" $ do
    ...
    it "aligns coloured and uncoloured lines identically" $ do
      let m' = msg LogInfo ["llm"] "llm dispatch" (object ["a" .= (1 :: Int)])
          stripped = T.filter (/= '\ESC') (T.concat (T.splitOn "[0m" (T.concat (T.splitOn "[2m" (T.concat (T.splitOn "[36m" (formatLogMessage utc True m')))))))
      stripped `shouldBe` plain m'

-- | The clock is rendered in the configured display timezone, not UTC.
--
-- It matters more than it looks: @journalctl -o cat@ is the only output
-- format that keeps ANSI colour, and it prints no timestamp prefix of
-- its own — so this is the only clock on a coloured production log, and
-- a UTC one is 8 hours off everything else on the box.
timezoneSpec :: Spec
timezoneSpec = describe "Max.Log.formatLogMessage clock" $ do
  it "renders in the configured zone, not UTC" $
    T.take 8 (formatLogMessage (minutesToTimeZone 480) False (msg LogInfo [] "x" Null))
      `shouldBe` "22:32:07"

  it "still reads as UTC when that is what was configured" $
    T.take 8 (plain (msg LogInfo [] "x" Null)) `shouldBe` "14:32:07"

  -- Zones west of UTC roll the date backwards; the line keeps the
  -- wall-clock time and no date, which is the point.
  it "handles a negative offset" $
    T.take 8 (formatLogMessage (minutesToTimeZone (-300)) False (msg LogInfo [] "x" Null))
      `shouldBe` "09:32:07"
